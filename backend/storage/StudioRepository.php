...
final class StudioRepository
{
    ...
    private function loadExercises(string $workspaceId): array
    {
        ...
        $skills = ['Foundation', 'Listening', 'Writing', 'Speaking', 'Reading'];
        $legacySkillMap = ['Grammar' => 'Foundation', 'Vocabulary' => 'Foundation'];
        $items = [];

        foreach ($skills as $skill) {
            $path = $base . DIRECTORY_SEPARATOR . $skill . DIRECTORY_SEPARATOR . 'exercises.json';
            $items = array_merge($items, array_map(fn (array $exercise): array => array_merge($exercise, ['skill' => $skill]), $this->readJson($path, [])));
        }

        foreach ($legacySkillMap as $legacySkill => $targetSkill) {
            $path = $base . DIRECTORY_SEPARATOR . $legacySkill . DIRECTORY_SEPARATOR . 'exercises.json';
            $legacyItems = array_map(fn (array $exercise): array => array_merge($exercise, ['skill' => $targetSkill]), $this->readJson($path, []));
    ...
    private function saveExercises(string $workspaceId, array $exercises): void
    {
        $skills = ['Foundation', 'Listening', 'Writing', 'Speaking', 'Reading'];
        foreach ($skills as $skill) {
            $items = array_values(array_filter($exercises, fn (array $exercise): bool => $this->normalizeSkill((string)($exercise['skill'] ?? 'Foundation')) === $skill));
            $items = array_map(fn (array $exercise): array => array_merge($exercise, ['skill' => $skill]), $items);
            $this->writeJson($this->dataFile('luyen-tap', $workspaceId, $skill . DIRECTORY_SEPARATOR . 'exercises.json'), $items);
        }

        foreach (['Grammar', 'Vocabulary'] as $legacySkill) {
            $this->writeJson($this->dataFile('luyen-tap', $workspaceId, $legacySkill . DIRECTORY_SEPARATOR . 'exercises.json'), []);
        }
    }

    private function normalizeSkill(string $skill): string
    {
        return in_array($skill, ['Grammar', 'Vocabulary', ''], true) ? 'Foundation' : $skill;
    }
        ...
        if (isset($data['workspaces']) && is_array($data['workspaces'])) {
            return [
                'settings' => array_merge($default['settings'], $data['settings'] ?? [], [
                    'selectedPracticeSkill' => $this->normalizeSkill((string)($data['settings']['selectedPracticeSkill'] ?? 'Foundation')),
                ]),
                'workspaces' => array_values(array_map(function (array $workspace): array {
                    return [
                        ...
                        'exercises' => array_values(array_map(fn (array $exercise): array => array_merge($exercise, [
                            'skill' => $this->normalizeSkill((string)($exercise['skill'] ?? 'Foundation')),
                        ]), $workspace['exercises'] ?? [])),
