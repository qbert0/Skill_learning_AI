(function registerListeningDictationExerciseType() {
    class ListeningDictationExerciseType extends PracticeExerciseType {
        constructor() {
            super(PracticeExerciseTypeDefinitionRegistry.require("listening_dictation"));
        }
                    ...
                    <div class="runner-question-items">
                        ${this.items(question).map((item, itemIndex) => `
                            <article class="runner-question-item runner-item-card">
                                ...
                                <input class="short-answer-input" type="text" data-question-id="${escapeHtml(`${question.id}::${item.id || itemIndex}`)}" placeholder="Nhập nội dung bạn nghe được">
                            </article>
                        `).join("")}
                    </div>
                </section>
        ...
        renderBuilderItem({ scope, index = 0, item = {} }) {
            ...
            return `
                <section class="question-item-editor" data-${scope}-item>
                    ...
                    <label>
                        <span>Chỉ dẫn / ngữ cảnh</span>
                        <input type="text" data-${scope}-item-prompt value="${escapeHtml(item.prompt || "")}" placeholder="Listen to the sentence and write it down.">
                    </label>
                    <label>
                        <span>Nội dung chuẩn</span>
                        <input type="text" data-${scope}-item-expected value="${escapeHtml(item.expected || "")}" placeholder="The expected transcript">
                    </label>
                    <label>
                        <span>Giải thích</span>
                        <textarea rows="2" data-${scope}-item-explanation placeholder="Mẹo nghe hoặc chính tả">${escapeHtml(item.explanation || "")}</textarea>
                    </label>
                </section>
            `;
        }
    ...
    PracticeExerciseTypeRegistry.register(new ListeningDictationExerciseType());
})();
